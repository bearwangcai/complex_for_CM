...
def data():
    A_P = xlrd.open_workbook(r'E:\复杂度分析\3-antenna.xls')
    ...
    node_v_angle = [0]*9
    P_P = xlrd.open_workbook(r"E:\复杂度分析\point.xls")
    P_P_table = P_P.sheets()[0]
    point_x = P_P_table.col_values(0)
    point_y = P_P_table.col_values(1)
...
def find_gain():
    Gain = xlrd.open_workbook(r'E:\复杂度分析\angle.xlsx')
...
def write_excel(Num):     
    ...
    for i in range(0,len(Num)):         
        sheet1.write(i,0,point_x[i])         
        sheet1.write(i,1,point_y[i])        
        sheet1.write(i,2,Num[i])          
    f.save(r"E:\复杂度分析\采样度覆盖次数.xls") 
